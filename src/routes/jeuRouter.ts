...
export class JeuRouter {
  ...
  public redemarrerJeu(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      // Invoquer l'opération système (du DSS) dans le contrôleur GRASP
      const resultat = this._controleurJeu.redemarrerJeu(nom);
      req.flash('info', `Redmarer le jeu`);
      res.status(200)
        .send({
          message: 'Success',
          status: res.status,
  ...
  init() {
    ...
    this._router.get('/redemarrerJeu/:nom', this.redemarrerJeu.bind(this)); // pour .bind voir https://stackoverflow.com/a/15605064/1168342
  }
